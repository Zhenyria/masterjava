...
public class MailSender {
    ...
    static MailResult sendTo(Addressee to,
                             String subject,
                             String body,
    ...
    static String sendToGroup(Set<Addressee> to,
                              Set<Addressee> cc,
                              String subject,
                              String body,
                              List<Attachment> attachments) throws WebStateException {
        ...
        try {
            ...
            if (attachments != null && !attachments.isEmpty()) {
                for (Attachment attachment : attachments) {
                    email.attach(attachment.getAttachment().getDataSource(),
                                 attachment.getName().replaceAll("\\P{ASCII}", "?"),
                                 null,
                                 ATTACHMENT);
                }
            }
            ...
            email.setHeaders(ImmutableMap.of("List-Unsubscribe",
                                             "<mailto:masterjava@javaops.ru?subject=Unsubscribe&body=Unsubscribe>"));
